...
def main():
    ...
    # cnn.add(keras.layers.Conv2D(32, kernel_size=(3, 3), activation="relu", input_shape=IMAGE_SIZE + (1, )))
    ...
    # cnn.fit(expanded_train_data, one_hot_train_label, batch_size=100, epochs=20, verbose=True, validation_data=(expanded_test_data, one_hot_test_label))
    ...
    # mlp.add(keras.layers.Dense(1024, activation="relu"))
    ...
    # mlp.fit(expanded_train_data, one_hot_train_label, batch_size=512, epochs=1000, verbose=True, validation_data=(expanded_test_data, one_hot_test_label))
    ...
    # evaluate(fit_and_predict(linear_model.PassiveAggressiveClassifier(C=0.8), flatten_train_data, train_label, flatten_test_data), test_label, "Passive Aggressor Classifier", )
    ...
    # evaluate(fit_and_predict(ensemble.RandomForestClassifier(n_jobs=4, n_estimators=100, max_depth=38), flatten_train_data, train_label, flatten_test_data), test_label, "random forest", )

    # evaluate(fit_and_predict(tree.DecisionTreeClassifier(max_depth=37), flatten_train_data, train_label, flatten_test_data), test_label, "decision tree")

    # evaluate(fit_and_predict(neural_network.MLPClassifier, flatten_train_data, train_label, flatten_test_data, hidden_layer_sizes=(512, 64), verbose=True, tol=1e-3, learning_rate_init=1e-3, shuffle=True, alpha=1e-3, batch_size=64), test_label, "Multi-Layer Perception", )
    ...
    # evaluate(fit_and_predict(linear_model.SGDClassifier(n_iter=200, shuffle=True, n_jobs=4, warm_start=True), flatten_train_data, train_label, flatten_test_data), test_label, "SGD Classifier")
    ...
    # RBM->Logistic
    ####################################################################################################################
    evaluate(fit_and_predict(linear_model.LogisticRegression, transformed_train_data, train_label, transformed_test_data, C=2000.0), test_label, "RBM-Logistic Classifier")

    ####################################################################################################################
    # RBM->RandomForest
    ####################################################################################################################
    evaluate(fit_and_predict(ensemble.RandomForestClassifier, transformed_train_data, train_label, transformed_test_data, n_estimators=100, max_depth=32), test_label, "RBM-random Forest Classifier")
