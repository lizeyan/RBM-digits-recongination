...
def main():
    ...
    rbm = BinaryRBM(n_hidden_units=transformed_size[0] * transformed_size[1], activation_function="relu", n_epochs=1000, batch_size=128, optimization_algorithm="sgd", learning_rate=3e-3)
    rbm.fit(flatten_train_data)
    rbm.fit(flatten_test_data)
    transformed_train_data = rbm.transform(flatten_train_data)
    transformed_test_data = rbm.transform(flatten_test_data)
